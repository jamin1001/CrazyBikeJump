...
namespace ShapeFX
{
	...
	public class ShFX_EffectHandler : MonoBehaviour
	{
		...
		// Reset static values if domain/scene reloading are disabled
		[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
		static void InitStaticValues()
		{
			lastShakeFrame = -1;
			curFrameMaxStrength = -1;
		}
#endif
		...
		void OnDisable()
		{
			...
			{
				/*Camera.onPreRender*/ RenderPipelineManager.beginCameraRendering -= OnCamPreRender;
				/*Camera.onPostRender*/ RenderPipelineManager.endCameraRendering -= OnCamPostRender;
			}
		}
        ...
        void OnCamPreRender(ScriptableRenderContext context, Camera cam)
		{
			if(cameraShaking)
			{
		...
		void OnCamPostRender(ScriptableRenderContext context, Camera cam)
